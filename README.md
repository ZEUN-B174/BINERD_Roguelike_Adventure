# 개요
여러분은 쿠르디스탄의 천재 프로그래머 다스니 박사의 조수가 되어 그녀의 실험을 도와주게 됩니다. 3개의 층으로 이루어진 시뮬레이션 던전에 입장해 각 층마다 있는 보스와 싸워 승리를 거머쥐어 봅시다! 잠깐 …뭐? 버그가 있다고…? 그치만 이건 아직 베타 버전인걸! 붕괴: 스타레일의 시뮬레이션 우주를 모티브로 만든 로그라이크 게임입니다. 어쩌면 스타레일에서 보았던 아이템이 여기에 있을수도? 작품명인 비네르드(BINERD)는 쿠르드어로 던전이라는 뜻으로, 여러 개의 선택지를 선택하며 탐험하는 것이 마치 던전의 여러 방들을 탐험하는듯 해 이름을 이렇게 지었습니다.
...
<br>**9월 22일:** 드디어 이 게임을 다시 건들기 시작했고... 노래 몇 개를 추가했습니다. 얼마나 게으른거야!!!
<br>**2026년 1월 11일:** ;;; 코드가 이상해서 싹 고쳤고, 이상한 아이템 클래스를 그냥 없애버렸습니다!! (언젠가 다시 넣겠죠?)
## 0.2 버전
### 0.2.1 버전
**1월 14일:** 마참내! 전투 시스템을 구현하는 것에 성공했습니다! 현재 적 리스트는 임시로 집어넣었고, 나중에 다른 적들로 교체할 예정입니다.
<br>**1월 15일:** GUI를 드디어 이쁘게 구현하는 것에 성공했고, 전반적으로 코드 리팩토링을 했습니다. 그리고 버프 시스템 구현하다가 킹받아서 때려쳤습니다. 자러갈꺼임 ㅅㄱ
<br>**1월 16일:** 먼저 전투를 시스템화해서 정예, 보스 구역에서도 같은 시스템을 사용할 수 있게 만들었습니다. (사건에서 전투로 넘어가는 경우에도 사용 할 수 있습니다.) 또한 어제 만들다 만 버프 시스템을 구현했고, 드디어 전투 시스템에서 연속 공격이 제대로 나오게 만들었습니다. 그리고 플레이어 캐릭터 상호작용도 보이게 구현했습니다. 나중에 적 이미지도 추가해 구현할 예정입니다. 아참, 전투랑 보상 받는것도 둘로 나눴습니다.
